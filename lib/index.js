...
(function() {
  var Buffer, Callback, Channel, Generator, Merger, Multicast, Process, Selector, alias, chan, final, fn, go, i, len, merge, mult, name, offer, poll, proc, receive, ref, ref1, select, send, sleep, timeout,
  ...
  chan.lift = function(fn, options) {
    var error, singular;
    if (options != null) {
      error = options.error, singular = options.singular;
    }
    if (error == null) {
      error = true;
    }
    ...
    return function() {
      var args, callback, ch;
      args = 1 <= arguments.length ? slice.call(arguments, 0) : [];
      ch = chan.single();
      callback = error ? singular ? function(err, value) {
        if (err != null) {
          throw err;
        }
        return send.async(ch, value);
      } : function() {
        var err, values;
        err = arguments[0], values = 2 <= arguments.length ? slice.call(arguments, 1) : [];
        if (err != null) {
          throw err;
        }
        return send.async(ch, values);
      ...
      fn.call.apply(fn, [this].concat(slice.call(args), [callback]));
      return ch;
    };
  };

  chan.from = function(array) {
    return chan(Buffer.from(array));
  };
  ...
  receive = function() {
    switch (arguments.length) {
      case 0:
        receive.$0.apply(receive, arguments);
        break;
      case 1:
        receive.$1.apply(receive, arguments);
        break;
      default:
  ...
  send = function() {
    switch (arguments.length) {
      case 1:
        send.$1.apply(send, arguments);
        break;
      case 2:
        send.$2.apply(send, arguments);
        break;
      default:
        throw new Error("Arity");
    }
  ...
  merge = function(inputs, output) {
    return (new Merger(inputs, output)).output;
  };
